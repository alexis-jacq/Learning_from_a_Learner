...
#----------------------------------------------------------
# halfcheetah:
# observer_steps = list(range(20,40))
# reacher:
# observer_steps = list(range(10,20))
# ant:
# observer_steps = [10,15,20,25,30]
# hopper:
# observer_steps = [10,15,20,25,30]

observer_steps = list(range(0, 5))

# hyperparameters used in the paper:
#-----------------------------------
entropy_coef = 0.01
gamma = 0.99
n_demos_per_update = 100
demos_dir = args.demos_dir

n_epoch_policies = 3000
n_epoch_kl = 100
...
actions_file_name_0 = demos_dir + '/actions_step_' + str(0) + '.npy'
states_file_name_0 = demos_dir + '/states_step_' + str(0) + '.npy'
...
class KL(nn.Module):
  ...
  def forward(self, state, k):
    hidden_kl = torch.tanh(self.fc1(state))
...
def main():
  ...
  for k in observer_steps:
    ...
    for epoch in range(n_epoch_policies):
      ...
      actions_file_name = demos_dir + '/actions_step_' + str(k) + '.npy'
      states_file_name = demos_dir + '/states_step_' + str(k) + '.npy'
  ...
  torch.save(last_policy_sd, args.policies_dir+'/last_policy_'+args.expe+'.pth')

  policies = []
  for pi_state_dict in policies_sd:
    pi = Policy([state_dim], action_space, base_kwargs={'recurrent': False})
  ...
  # ------------------------------
  reward = Policy([state_dim], action_space, base_kwargs={'recurrent': False})
  ...
  torch.save(reward.state_dict(),
             args.rewards_dir + '/initial_reward_' + args.expe + '.pth')
  ...
  #------------------
  torch.save(
      reward.state_dict(), args.rewards_dir + '/reward_' + args.expe + '.pth')
